...
static void inference(AppInferenceContext *context) {
    ...
    int num_workers = (context->network != nullptr) ? context->network->nSockets : 0;
	...
	diag.printEval(evalTime, syncTime, num_workers, sentBytes, recvBytes, batchSize);
	...
	diag.resetPerToken();
        ...
        NnUint syncTime = context->executor->getTotalTime(STEP_SYNC_NODES);
        diag.printPred(predTime, syncTime, num_workers, sentBytes, recvBytes, piece);
	fflush(stdout);
