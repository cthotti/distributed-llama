...
void NnNetwork::writeMany(NnUint n, NnSocketIo *ios) {
    bool isWriting;
    std::vector<long long> socketStartUs(n);
    std::vector<NnSize> originalSizes(n);
    long long now = diagNowUs();
    for (NnUint i = 0; i < n; i++) {
        socketStartUs[i] = now;
        originalSizes[i] = ios[i].size;
        assert(ios[i].socketIndex < nSockets);
    ...
    do {
        ...
        for (NnUint i = 0; i < n; i++) {
            ...
            if (io->size > 0) {
                ...
                if (s < 0) {
                    if (isEagainError()) {
			long long elapsed = diagNowUs() - socketStartUs[i]; 
			if(elapsed > 500000){
				char ts[16];
				diagTimestamp(ts, sizeof(ts));
				fprintf(stderr, "⚠️  [%s] SLOW SEND socket=%u stalled=%lld ms sent=%zu/%zu bytes\n",
                                	ts,
                                	io->socketIndex,
                                	elapsed / 1000,
                                	originalSizes[i] - io->size,
                                	originalSizes[i]);
                            	fflush(stderr);
                            	socketStartUs[i] = diagNowUs();
			}
                        continue;
                    }
...
static void syncNodeSlices(bool onlyFromWorkerToRoot, NnNetwork *network, NnUint nodeIndex, NnUint nNodes, NnByte *buffer, NnSize nBytes, NnUint nThreads, NnUint threadIndex) {
    ...
    if (!onlyFromWorkerToRoot || !isWorker) {
        ...
        for (NnUint i = 0; i < nSocketsPerThread; i++) {
    		long long t0 = diagNowUs();
    		network->readMany(1, &ios[i]);
    		long long elapsed = diagNowUs() - t0;
    		if (elapsed > 500000) { // > 500ms
        		char ts[16];
        		diagTimestamp(ts, sizeof(ts));
        		fprintf(stderr, "⚠️  [%s] SLOW WORKER socket=%u waited=%lld ms thread=%u\n",
            			ts,
            			ios[i].socketIndex,
            			elapsed / 1000,
            			threadIndex);
