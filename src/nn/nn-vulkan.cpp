...
std::pair<vk::Buffer, VmaAllocation> NnVulkanContext::createRawBuffer(const vk::DeviceSize bufferSize, const vk::BufferUsageFlags usageFlags, bool hostVisible) {
    ...
    printf("🌋 VMA allocating buffer: %lu MB (hostVisible=%d)\n",
        (unsigned long)bufferSize / (1024*1024), hostVisible);
    //fflush(stdout);
    ...
    if (result != VK_SUCCESS)
        throw std::runtime_error("VMA failed to allocate buffer of size " +
            std::to_string(bufferSize / (1024*1024)) + " MB, error: " + std::to_string(result));
...
void NnVulkanStagingCopier::tryRelease() {
    if (allocatedSize > 0u) {
        vmaUnmapMemory(context->allocator, hostAllocation);
        vmaDestroyBuffer(context->allocator, hostBuffer, hostAllocation);
        hostAllocation = VK_NULL_HANDLE;
        allocatedSize = 0u;
    }
}
...
void NnVulkanStagingCopier::allocate(const NnSize size) {
    if (allocatedSize != size) {
        tryRelease();
        auto b = context->createRawBuffer(size,
            vk::BufferUsageFlagBits::eTransferSrc | vk::BufferUsageFlagBits::eTransferDst,
            true);
        hostBuffer = b.first;
        hostAllocation = b.second;
        vmaMapMemory(context->allocator, hostAllocation, &hostPointer);
        allocatedSize = size;
    }
}
    ...
    usageFlags(usageFlags)
{
    ...
    isHostVisible = fastAccess;

    VULKAN_TRACE("Creating buffer of size %zu (fastAccess=%d)", bufferSize, fastAccess);

    auto b = context->createRawBuffer(bufferSize, usageFlags, fastAccess);
    deviceBuffer = b.first;
    deviceAllocation = b.second;

    if (fastAccess) {
        VmaAllocationInfo allocInfo;
        vmaGetAllocationInfo(context->allocator, deviceAllocation, &allocInfo);
        if (allocInfo.pMappedData != nullptr) {
            hostPointer = (NnByte *)allocInfo.pMappedData;
        } else {
            void *mapped = nullptr;
            VkResult r = vmaMapMemory(context->allocator, deviceAllocation, &mapped);
            if (r == VK_SUCCESS) {
                hostPointer = (NnByte *)mapped;
            } else {
                isHostVisible = false;
            }
        }
...
NnVulkanBuffer::~NnVulkanBuffer() {
    if (hostPointer != nullptr && isHostVisible)
        vmaUnmapMemory(context->allocator, deviceAllocation);
    vmaDestroyBuffer(context->allocator, deviceBuffer, deviceAllocation);
...
void NnVulkanBuffer::write(const NnByte *data, const NnSize offset, const NnSize size) {
    ...
    if (offset + size > bufferSize) {
        printf("🚨 write assertion fail: name=%s offset=%zu size=%zu bufferSize=%zu\n",
            name, offset, size, bufferSize);
        fflush(stdout);
    }
    ...
    if (isHostVisible && hostPointer != nullptr) {
        ...
        vmaFlushAllocation(context->allocator, deviceAllocation, offset, size);
...
void NnVulkanBuffer::read(NnByte *data, const NnSize offset, const NnSize size) {
    ...
    if (isHostVisible && hostPointer != nullptr) {
        vmaInvalidateAllocation(context->allocator, deviceAllocation, offset, size);
        std::memcpy(data, hostPointer, size);
...
static void buildShaderLayout(std::vector<NnOpBufferAccess> &a, NnVulkanDeviceData *data, NnVulkanDeviceSegmentData *segmentData, NnUint opIndex, NnOpConfig *opConfig) {
    ...
    if (opConfig->weightSize.nBytes > 0) {
        NnUint baseIndex = segmentData->resolveOpWeightBufferBaseIndex(opIndex);
        a.push_back({ACCESS_IMMUTABLE, data->internalBuffers[baseIndex].get()});
        NnUint chunkSize = segmentData->weightChunkSizes[opIndex];
        if (chunkSize > 0) {
            NnSize totalSize = opConfig->weightSize.nBytes;
            NnSize blockSize = getBlockSize(opConfig->weightSize.floatType);
            NnSize chunkBytes = (NnSize)chunkSize * blockSize;
            NnUint nChunks = (totalSize + chunkBytes - 1) / chunkBytes;
            for (NnUint c = 1; c < nChunks; c++) {
                if (baseIndex + c < data->internalBuffers.size())
                    a.push_back({ACCESS_IMMUTABLE, data->internalBuffers[baseIndex + c].get()});
            }
        }
    }
    // config
...
NnUint NnVulkanDeviceSegmentData::resolveOpWeightBufferBaseIndex(NnUint opIndex) {
    assert(weightBufferIndex[opIndex] != UINT32_MAX);
    return weightBufferIndex[opIndex];
}
...
static std::vector<NnUint> resolveShaderConstants(const NnOpConfig *opConfig, const NnUint nBatches, const NnSize3D inputSize, const NnSize3D outputSize, NnUint weightChunkSize = 0) {
    ...
    if (opConfig->code == OP_EMBEDDING) {
        consts.push_back(weightChunkSize);
    }
    return consts;
}
...
NnVulkanDeviceSegmentData::NnVulkanDeviceSegmentData(NnVulkanBufferFactory *bufferFactory, NnVulkanDeviceData *data, NnSegmentConfig *segmentConfig, NnUint nBatches) :
    ...
    weightChunkSizes(segmentConfig->nOps, 0),
    configBufferIndex(segmentConfig->nOps, UINT32_MAX)
{
    for (NnUint opIndex = 0; opIndex < segmentConfig->nOps; opIndex++) {
        ...
        // chet's implemented split weight buffer creation
	if (opConfig->weightSize.nBytes > 0) {
            const NnSize MAX_BUFFER_SIZE = 500 * 1024 * 1024; // 900MB - safely under 1GB limit
            const NnSize totalSize = opConfig->weightSize.nBytes;
            if (totalSize <= MAX_BUFFER_SIZE) {
                // Single buffer - fits within limit
                data->internalBuffers.push_back(
                    bufferFactory->create("weights", totalSize, false, vk::BufferUsageFlagBits::eStorageBuffer, false)
                );
                weightBufferIndex[opIndex] = data->internalBuffers.size() - 1;
                // Set chunkSize to 0 meaning single buffer mode
                if (opConfig->configSize >= sizeof(NnMatmulOpConfig)) {
                    NnMatmulOpConfig *cfg = (NnMatmulOpConfig *)opConfig->config;
                    cfg->weightChunkSize = 0;
                }
            } else {
                // Split into chunks
                const NnSize blockSize = getBlockSize(opConfig->weightSize.floatType);
                const NnSize totalBlocks = totalSize / blockSize;
                const NnSize blocksPerChunk = (MAX_BUFFER_SIZE / blockSize);
                const NnSize chunkSize = blocksPerChunk * blockSize;
                const NnSize chunk1Size = std::min(chunkSize, totalSize);
                const NnSize chunk2Size = std::min(chunkSize, totalSize > chunkSize ? totalSize - chunkSize : 0);
                const NnSize chunk3Size = totalSize > 2 * chunkSize ? totalSize - 2 * chunkSize : 0;

                printf("🌋 Splitting weight buffer: %zu MB into chunks of %zu MB\n",
                    totalSize / (1024*1024), chunkSize / (1024*1024));

		weightBufferIndex[opIndex] = data->internalBuffers.size();
                for (NnSize off = 0; off < totalSize; off += chunkSize) {
                    NnSize sz = std::min(chunkSize, totalSize - off);
                    data->internalBuffers.push_back(
                        bufferFactory->create("weightsChunk", sz, false, vk::BufferUsageFlagBits::eStorageBuffer, false)
                    );
                }

                // Store chunk size in blocks for shader
                if (opConfig->configSize >= sizeof(NnMatmulOpConfig)) {
                    NnMatmulOpConfig *cfg = (NnMatmulOpConfig *)opConfig->config;
                    cfg->weightChunkSize = blocksPerChunk;
                }
		weightChunkSizes[opIndex] = (NnUint)blocksPerChunk;
            }
        }
    ...
    buffersToSync(segmentConfig->nOps)
{
    ...
    for (NnUint opIndex = 0; opIndex < segmentConfig->nOps; opIndex++) {
        ...
        constants[opIndex] = resolveShaderConstants(opConfig, netConfig->nBatches, inputSize, outputSize, segmentData->weightChunkSizes[opIndex]);
...
void NnVulkanDeviceSegment::loadWeight(NnUint opIndex, NnSize offset, NnSize nBytes, NnByte *weight) {
    ...
    if (chunkSize > 0) {
        const NnSize blockSize = getBlockSize(opConfig->weightSize.floatType);
        const NnSize chunkBytes = (NnSize)chunkSize * blockSize;
        NnUint baseIndex = segmentData->resolveOpWeightBufferBaseIndex(opIndex);
        NnUint nChunks = (opConfig->weightSize.nBytes + chunkBytes - 1) / chunkBytes;
        printf("🌋 loadWeight chunk: baseIndex=%u nChunks=%u chunkBytes=%zu totalBuffers=%zu\n",
            baseIndex, nChunks, chunkBytes, data->internalBuffers.size());
        fflush(stdout);
	NnSize remaining = nBytes;
        NnSize srcOffset = 0;
        NnSize dstOffset = offset;
        while (remaining > 0) {
            NnUint chunkIndex = dstOffset / chunkBytes;
            NnSize chunkOff = dstOffset % chunkBytes;
            NnSize writeSize = std::min(remaining, chunkBytes - chunkOff);
            data->internalBuffers[baseIndex + chunkIndex]->write(weight + srcOffset, chunkOff, writeSize);
            srcOffset += writeSize;
            dstOffset += writeSize;
            remaining -= writeSize;
        }
