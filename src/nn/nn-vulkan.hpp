...
public:
    ...
    ~NnVulkanContext();
    std::pair<vk::Buffer, VmaAllocation> createRawBuffer(const vk::DeviceSize bufferSize, const vk::BufferUsageFlags usageFlags, bool hostVisible);
};
...
public:
    std::vector<NnUint> weightChunkSizes;
    ...
    NnUint resolveOpWeightBufferBaseIndex(NnUint opIndex);
};
